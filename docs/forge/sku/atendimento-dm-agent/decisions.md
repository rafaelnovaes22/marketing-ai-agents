...
6. **DPA obrigatório** entre Acme e cada tenant antes de operação externa (BLOQUEADO em ADR-004-DM).

**Em SHADOW interno** (Acme staff only):
...
Em conversa com founder em 2026-05-13, foi explicitado que **Acme NÃO possui:**
...
Atendimento DM é o **único SKU criticality A** que processa dados pessoais de terceiros (clientes do tenant) em tempo real. Operar este SKU para clientes externos sem DPO + DPA + opt-in flow validado expõe Acme a multa material, dano reputacional e cancelamento da operação.

### Decisão

**SHADOW INTERNO APENAS.** Operação restrita a:

1. **Apenas Acme staff** como "usuário" — assinou opt-in explícito + ciente de uso de IA
...
6. **Bloqueio mecânico** em `/acme:promote --to=assisted` (hook pre-promote retorna erro)
...
1. **Risco regulatório material:** 2% do faturamento é potencialmente catastrófico para Acme em estágio inicial.
...
- ✅ Quando habilitar ASSISTED externo, Acme estará compliant + diferenciada vs concorrentes
