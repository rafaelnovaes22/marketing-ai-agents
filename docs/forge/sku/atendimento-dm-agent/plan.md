...
6. **SHADOW interno + LGPD opt-in** (0.5 dia) — apenas Acme staff, sem dados reais de cliente
...
│  • RetainOrForgetUseCase (LGPD 90d + direito esquecimento)   │
└────────────────────┬────────────────────────────────────────┘
                     │
┌────────────────────▼────────────────────────────────────────┐
...
| Migrations + seed (Acme staff users opt-in) | Prisma | A |
...
| `RetainOrForgetUseCase` (LGPD 90d + delete via "esqueça meus dados") | application | A |
...
| Item | Detalhe |
|------|---------|
| Test RED via `/acme:aios-run --step=test --mode=red` | tests/atendimento-dm-agent/{unit,integration,e2e,adversarial}/ |
...
| Item | Detalhe |
|------|---------|
| `/acme:aios-run --step=test --mode=verify` | Cobertura + gaps |
...
| CI workflow `forge-eval-dm` ativo | Roda a cada PR |

## 8. Fase 6 — SHADOW INTERNO + LGPD opt-in (0.5 dia)

> ⚠️ **NOTA CRÍTICA:** SHADOW interno significa **APENAS Acme staff** respondendo DMs simulados/internos com opt-in assinado. **ZERO dados de cliente real externo.**

### Entregáveis

| Item | Detalhe |
|------|---------|
| Opt-in form criado + assinado por staff Acme | Custom + audit log |
| Disclaimer LGPD ativo: "Atendimento usa IA, retenção 30 dias em SHADOW" | Custom |
| Webhook conectado apenas em conta IG/FB/WhatsApp **interna** Acme | Meta Business |
| Cobrança DESLIGADA (R$ 0 em SHADOW interno) | Feature flag |
| `/acme:promote atendimento-dm-agent --to=shadow_internal` | promotion-officer assina |
...
- ✅ Acme staff envia DMs de teste, recebe respostas em <10s
- ✅ BANT registrado, mas CRM em modo dry-run (não cria deal real)
- ✅ Langfuse mostra 100% das conversas com `tenant=acme_internal` e `mode=shadow_internal`
...
## 9. Workflow AIOS TDD-first (Forge-10) adaptado para criticality A
...
# 2. Schema Prisma
/acme:aios-run atendimento-dm-agent --step=schema

# 3. Test RED
/acme:aios-run atendimento-dm-agent --step=test --mode=red
# Operador roda `npm test` e CONFIRMA QUE FALHA

# 4. Build
/acme:aios-run atendimento-dm-agent --step=build

# 5. Test VERIFY (Tier A: ≥90% line)
/acme:aios-run atendimento-dm-agent --step=test --mode=verify
...
# 7. Eval
/acme:eval atendimento-dm-agent
# Critério: ≥85% pass rate (≥22/25)

# 8. Review
/acme:aios-run atendimento-dm-agent --step=review

# 9. Promote (BLOQUEADO PARA EXTERNO)
/acme:promote atendimento-dm-agent --to=shadow_internal
# /acme:promote --to=assisted RETORNA ERRO até ADR-004-DM resolvido
```
...
| Eval-suite pass rate | `/acme:eval` | ≥ 85% (≥22/25) |
...
→ `/acme:tasks atendimento-dm-agent` (decomposição em 6 ondas — ver `tasks.md`)
