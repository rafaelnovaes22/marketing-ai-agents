...
- [ ] **T1.3** Migrations + seed (Acme staff users com opt-in) — 30min — dev — T1.2
...
- [ ] **T2.13** `RetainOrForgetUseCase` (LGPD 90d + comando "esqueça meus dados" deleta tudo) — 1.5h — dev — T2.1, T1.10

**Total Wave 2:** ~20h (1.5-2 dias)

## Wave 3 — Test RED (TDD-first Forge-10) + Eval seed (0.5 dia)

> ⚠️ **Gate G6 obriga:** testes RED **antes** de Wave 4 (build refinement).

- [ ] **T3.1** `/acme:aios-run --step=test --mode=red` → gera `tests/atendimento-dm-agent/{unit,integration,e2e,adversarial}/` — 30min — agent
...
- [ ] **T5.1** `/acme:aios-run --step=test --mode=verify` (cobertura ≥90% line Tier A) — 30min — agent
...
- [ ] **T5.6** CI workflow `forge-eval-dm` ativo (roda a cada PR) — 1h — dev
...
> ⚠️ **NÃO publicar em DMs de cliente real externo.** Apenas Acme staff com opt-in assinado.

- [ ] **T6.1** Opt-in form para staff Acme + assinaturas registradas em `OptInRecord` — 1h — dev
- [ ] **T6.2** Configurar webhook em conta IG/FB/WhatsApp **interna Acme** (não conta de cliente) — 1h — dev
...
- [ ] **T6.7** Hook pre-promote: `/acme:promote --to=assisted` retorna ERRO até ADR-004-DM resolvido — 1h — dev
- [ ] **T6.8** `/acme:promote atendimento-dm-agent --to=shadow_internal` (promotion-officer assina) — 30min — agent
