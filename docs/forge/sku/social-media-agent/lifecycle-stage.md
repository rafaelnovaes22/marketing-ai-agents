...
|-------|------|-----------|-----------------|-------|
| `(new)` → `draft` | 2026-05-13 | @po-guardian (pending review) | Diagnose preliminar + spec inicial | Primeiro SKU do Acme Social. Pipeline Forge-12 Fase 2 aplicado pela primeira vez em projeto real consumidor. |
...
4. [ ] Eval-suite com ≥ 85% pass rate (`/acme:eval social-media-agent`)
5. [ ] Coverage Tier B atingido (≥ 85% line, ≥ 80% branch)

**Comando para transição:** `/acme:promote social-media-agent --to=shadow`
...
**Comando:** `/acme:promote social-media-agent --to=assisted`

**Pré-requisito adicional:** `/acme:sla-threshold social-media-agent` define SLA contratual.
...
- Cliente piloto (Acme própria) endossa qualidade
...
- `/acme:promote social-media-agent --to=shadow` (de ASSISTED ou AUTONOMOUS)
- `/acme:promote social-media-agent --to=draft` (de SHADOW)
