...
### Brand
- `brand/acme-brand-guide.yaml` (paleta + tipografia + tolerância + validator rules)
...
3. **Subir Postgres local (Docker):**
   ...
   docker run -d \
     ...
     -e POSTGRES_DB=acme_social \
     -p 5432:5432 \
     postgres:16-alpine
   ```

4. **Configurar `DATABASE_URL`** no `.env` apontando para o Postgres acima (user=`dev`, password=`dev`, host=`localhost`, port=`5432`, db=`acme_social`).
...
# Verificar versão Forge consumida
bash scripts/forge version
# Esperado: Forge v0.12.0
```
...
✅ `bash scripts/forge version` → 0.12.0
⏳ ImagenAdapter.generate() jogará erro "Wave 2 / TDD red phase" — **isso é esperado**

---

## 🔴 Wave 3 — TDD Red Phase (próximo gate Forge-10)
...
> Gate G6 do Forge-10 (`tdd-red-phase-check` no CI) bloqueia merge se módulo modificado não tem `tests/{module}/unit/`.
...
**Sugestão para o Forge:** permitir whitelist por arquivo (`.forge-secret-scan-ignore`) OU adicionar regex específico para `localhost`/`example.com`/`dev:dev` que sinaliza intenção didática.
...
**Sugestão:** Forge adicionar comentário-convenção `// @forge:not-implemented-wave=2` que hook reconhece e suprime alertas.
