...
> ADRs específicos deste SKU. Decisões globais do projeto ficam em `docs/forge/decisions.md` raiz (a criar quando aparecerem). Convenção de IDs: **ADR-NNN-CW** (CW = Copywriter).
...
Forge C7 exige domain layer sem acoplamento a SDK específico. Adicionalmente, ADR-002-DS do social-media-agent já estabeleceu o padrão de adapter — este ADR confirma e estende para copywriter especificamente.
...
**Matriz de compatibilidade** mantida em `docs/forge/sku/copywriter-agent/handoff-contract.md`:
...
- **Cross-SKU:** decisões que afetam mais de 1 SKU vão para `docs/forge/decisions.md` raiz com prefixo `ADR-NNN-DS` (Acme Social geral)
