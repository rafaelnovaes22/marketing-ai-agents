...
diagnose_command: "/acme:diagnose copywriter-agent"
...
✅ **Positivo 2 (email sequence):** "Sequência de lançamento 4 emails — curso Acme Forge, tom direto, framework Soap Opera + One-CTA" → 4 emails (subject + preview + body + CTA), gancho narrativo conectado, 11m44s.
...
**Cabe no ICP?** ✅ Sim — Acme Social tem founder em fase de tração lançando 2-3 campanhas/mês como cliente fase 1 (single-tenant). Fase 2 expande para founders SaaS B2B com pipeline de lançamentos recorrentes.
...
→ `/acme:spec --type=platform-sku copywriter-agent`
