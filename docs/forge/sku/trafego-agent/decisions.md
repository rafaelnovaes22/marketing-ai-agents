...
> ADRs específicos deste SKU. Decisões globais do projeto ficam em `docs/forge/decisions.md` raiz. Convenção de IDs: **ADR-NNN-TF** (TF = Tráfego).
...
Acme precisa de algoritmo de bandit para realocação dinâmica de budget entre variantes de ad (3-5 por campanha) a cada 4h. Candidatos:
...
Volume típico das campanhas Acme varia de **8-100 conversões/dia** (mix de baixo e médio volume).
...
2. **Custo limitado (C3):** máximo 2 retries por campanha (~+R$ 1,00 absorvido pela Acme em <2% dos casos).
...
1. **Mitigação contratual:** quando Meta lança v20 com deprecation timeline (ex: 6 meses), Acme tem janela controlada para migrar.
...
> NÃO inventar endpoints Meta que não existem. Toda chamada documentada em `docs/forge/sku/trafego-agent/meta-api-endpoints.md` (a criar na Wave 1) com link para developer docs oficiais por versão.
...
Acme Social precisa decidir como cobrar:
...
- **Acme cobra R$ 50** por campanha publicada + ciclos de bandit (até 30 dias de vida da campanha).
- **Ad spend** (budget dos ads) é **separado**, debitado **direto do cartão Meta do cliente** vinculado ao Business Manager dele.
- Acme **NUNCA** intermedia ad spend (não recebe, não repassa).
...
1. **Sem incentivo perverso:** Acme não ganha mais cobrando mais ad spend → otimização honesta.
2. **Transparência total:** cliente vê ad spend no painel Meta dele — Acme não tem como inflar.
3. **Compliance fiscal/tributária:** Acme não atua como intermediário financeiro (evita Sefaz/Receita Federal classificar como instituição de pagamento).
4. **Escala sem caixa preso:** Acme não precisa de capital de giro para pagar Meta antes de cobrar cliente.
5. **Alinhado a unit-economics.md (Seção 5):** C3 calcula apenas custos Acme, não inclui ad spend.
...
- Demanda significativa por "modelo full-service" (Acme intermedia ad spend) → avaliar criar SKU separado com modelo C (hybrid) e estrutura legal apropriada
...
Cliente recebe mensagem explícita: *"Esta categoria (Special Ad Category) requer humano no loop por compliance Meta. Indisponível na fase 1 da Acme."*
...
→ Validar legal/compliance ADR-004-TF + ADR-005-TF antes de Wave 6 (`/acme:promote`)
