...
|-------|------|-----------|-----------------|-------|
| `(new)` → `draft` | 2026-05-13 | @po-guardian (pending review) | Diagnose + spec + unit-economics + plan | SKU P1 do Acme Social. Implementação prevista para D7 do roadmap 14d. |
...
4. [ ] Eval-suite com ≥ 85% pass rate (`/acme:eval trafego-agent`) — incluindo bandit convergência ≥70%
...
**Comando para transição:** `/acme:promote trafego-agent --to=shadow`
...
- 50+ campanhas criadas em SHADOW na conta Acme própria
...
**Comando:** `/acme:promote trafego-agent --to=assisted`

**Pré-requisito adicional:** `/acme:sla-threshold trafego-agent` define SLA contratual (300s setup, **não** tempo Meta aprovar).
...
- Cliente piloto (Acme própria) endossa qualidade subjetivamente
...
- `/acme:promote trafego-agent --to=shadow` (de ASSISTED ou AUTONOMOUS)
- `/acme:promote trafego-agent --to=draft` (de SHADOW)
...
### SHADOW
- ≥ 50 campanhas criadas/semana em conta Acme
