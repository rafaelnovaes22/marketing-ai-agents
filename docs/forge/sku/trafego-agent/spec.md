...
> **SKU cobra R$ 50 pela CRIAÇÃO + OTIMIZAÇÃO da campanha.** Ad spend (budget que vai para Meta) é **separado** e debitado direto do cartão Meta do cliente. Exemplo: cliente roda R$ 30.000/mês de spend → Acme cobra R$ 50 por cada nova campanha publicada + ciclo de bandit, não percentual sobre spend.
...
SHADOW (interno, ~Week 9 D1-D5)
  • 50+ campanhas criadas em conta Acme própria
...
> Cliente paga ad spend **separadamente** (não computado em C3 da Acme).
...
**Fase 1 (este SKU):** single-tenant — uma Business Manager Acme, um Mixpanel project.
