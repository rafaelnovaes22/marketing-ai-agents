...
diagnose_command: "/acme:diagnose trafego-agent"
...
**Quanto a dor custa:** R$ 8–15k/mês fixos + custo de oportunidade (campanhas sub-otimizadas queimam 20–40% do budget conforme benchmark interno Acme).
...
- Founders que rodam <R$ 3k/mês — overhead da Acme não compensa.
...
- **Single-tenant fase 1** (C8): única Business Manager (Acme). Multi-tenant exige tenant_id → Meta ad_account_id mapping.
...
→ `/acme:spec --type=platform-sku trafego-agent`
...
**Aprovação pendente:** `@po-guardian` revisar outcome contratual + separação contratual entre fee Acme e ad spend do cliente.
