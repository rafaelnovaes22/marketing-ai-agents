...
| Item | Detalhe |
|------|---------|
| `/acme:aios-run --step=test --mode=red` | Gera `tests/designer-agent/{unit,integration,e2e}/` |
| RED evidence commitada (Gate G6 Forge-10) | Operador confirma falha local |
...
| CI workflow `forge-eval` | Roda diariamente no SKU |
...
| Item | Stack |
|------|-------|
| `/acme:sla-threshold designer-agent` (SLA 20min, brand ≥99%) | Documento + workflow |
...
| `/acme:promote designer-agent --to=shadow` (promotion-officer assina) | Comando Forge |

### Output esperado fim Fase 4

- `bash scripts/forge doctor` (consumer mode) sem novos erros críticos
...
/acme:aios-run designer-agent --step=schema
...
/acme:aios-run designer-agent --step=build

# 5. Test VERIFY
/acme:aios-run designer-agent --step=test --mode=verify

# 6. Eval calibração
npm run eval designer-agent

# 7. Review final
/acme:aios-run designer-agent --step=review

# 8. Promoção SHADOW
/acme:promote designer-agent --to=shadow
```
...
| E2E Playwright (geração real, sandbox Imagen + Ideogram) | CI workflow `forge-test` | 2 happy paths + 1 retry path |
| TDD red phase evidence | Gate G6 mecânico | `tests/designer-agent/unit/` ≥ 1 arquivo antes do build |
| Eval-suite pass rate | `/acme:eval` | ≥ 88% |
...
→ `/acme:tasks designer-agent` (decomposição em waves executáveis)
