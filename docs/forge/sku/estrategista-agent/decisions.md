...
> ADRs específicos deste SKU. Decisões globais do projeto ficam em `docs/forge/decisions.md` raiz. Convenção de IDs: **ADR-NNN-EST** (EST = Estrategista).
...
**Thresholds:** **≥30 dias E ≥100 usuários ativos** no período. Calibrados em pesquisa de literatura de analytics (Lean Analytics + Reforge curriculum) — abaixo disso, padrões observados são frequentemente artefatos.
