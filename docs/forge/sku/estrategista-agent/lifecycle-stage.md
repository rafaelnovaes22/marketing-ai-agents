...
4. [ ] Eval-suite com ≥ 85% pass rate (`/acme:eval estrategista-agent`)
...
**Comando para transição:** `/acme:promote estrategista-agent --to=shadow`
...
- 30+ diagnósticos rodados em tenants piloto (incluindo Acme própria como tenant)
...
**Comando:** `/acme:promote estrategista-agent --to=assisted`

**Pré-requisito adicional:** `/acme:sla-threshold estrategista-agent` define SLA contratual de 120s P95.
...
- `/acme:promote estrategista-agent --to=shadow` (de ASSISTED ou AUTONOMOUS)
- `/acme:promote estrategista-agent --to=draft` (de SHADOW)
