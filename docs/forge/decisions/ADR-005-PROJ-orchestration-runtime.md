...
Os 7 SKUs do Acme Social têm perfis de orquestração heterogêneos:
...
- Human-in-the-loop declarativo (`interrupt_before` em node) — modo ASSISTED do Forge ganha implementação direta.
...
- [ ] Atualizar `.claude/commands/acme/pre-merge-check.md` G1: permitir `@langchain/langgraph` em `src/orchestration/` (não em `src/application/` nem `src/domain/`).
