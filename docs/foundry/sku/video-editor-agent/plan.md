...
**3-4 dias úteis** seguindo pipeline AIOS TDD-first (Foundry-10):
...
| `/novais-digital:aios-run --step=test --mode=red` gera testes que FALHAM | Gate G6 evidence |
...
| Item | Stack |
|------|-------|
| Test VERIFY (`/novais-digital:aios-run --step=test --mode=verify`) | Foundry-10 |
...
| `/novais-digital:sla-threshold video-editor-agent` (SLA contratual) | gate |
...
| `/novais-digital:promote video-editor-agent --to=shadow` | promotion-officer |
...
# Schema
/novais-digital:aios-run video-editor-agent --step=schema

# Test RED (testes ANTES do código)
/novais-digital:aios-run video-editor-agent --step=test --mode=red
# Operador roda npm test e CONFIRMA QUE FALHA

# Build
/novais-digital:aios-run video-editor-agent --step=build

# Test VERIFY
/novais-digital:aios-run video-editor-agent --step=test --mode=verify

# Review final
/novais-digital:aios-run video-editor-agent --step=review
```
...
| Eval pass rate | `/novais-digital:eval` | ≥ 85% |
...
→ `/novais-digital:tasks video-editor-agent` (decomposição em ClickUp)
