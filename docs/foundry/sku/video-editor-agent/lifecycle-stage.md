...
4. [ ] Eval-suite com ≥ 85% pass rate (`/novais-digital:eval video-editor-agent`)
5. [ ] Coverage Tier B atingido (≥ 85% line, ≥ 80% branch)
6. [ ] **Lint rule no-veo3 ativa em CI** (mecanismo de bloqueio do Cenário B)

**Comando para transição:** `/novais-digital:promote video-editor-agent --to=shadow`
...
- 50+ execuções coletadas em SHADOW (vídeos reais teste internos Novais Digital)
...
**Comando:** `/novais-digital:promote video-editor-agent --to=assisted`

**Pré-requisito adicional:** `/novais-digital:sla-threshold video-editor-agent` define SLA contratual.
...
- Cliente piloto (Novais Digital própria) endossa qualidade dos cortes
...
- `/novais-digital:promote video-editor-agent --to=shadow` (de ASSISTED ou AUTONOMOUS)
- `/novais-digital:promote video-editor-agent --to=draft` (de SHADOW)
