...
> ⚠️ **Foundry-10 obriga:** test_agent em mode=red gera testes ANTES de Wave 4 (build final).
> Operador roda `npm test` e CONFIRMA QUE FALHA antes de prosseguir.

- [ ] **T3.1** `/novais-digital:aios-run --step=test --mode=red` (gera tests/video-editor-agent/{unit,integration,e2e}/) — 30min — dev/agent
...
- [ ] **T5.7** CI workflow `foundry-test` ativo para este SKU + lint no-veo3 — 1h — dev

**Total Wave 5:** ~8h

## Wave 6 — Ship + SLA + Promoção SHADOW (1 dia)

- [ ] **T6.1** `/novais-digital:sla-threshold video-editor-agent` (SLA contratual 10min) — 30min — dev
...
- [ ] **T6.5** `/novais-digital:promote video-editor-agent --to=shadow` (promotion-officer assina) — 30min — agent
