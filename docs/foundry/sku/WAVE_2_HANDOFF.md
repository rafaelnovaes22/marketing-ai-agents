...
> **Próxima Wave:** 3 — Test RED (TDD-first, Foundry-10) ou direto para Wave 5 (Eval-suite) por SKU
...
| T3.1-T3.3 — TDD RED phase canônica (`/novais-digital:aios-run --step=test --mode=red`) | 3 | 1h |
...
**Sugestão para o Foundry:** o template `package.json` consumidor poderia usar `vitest --dir tests` ou globs específicos por SKU.
...
**Caminho A — TDD RED canônico (recomendado pelo Foundry):**
...
```bash
/novais-digital:eval social-media-agent  # tem mais testes; calibrar runner primeiro
/novais-digital:eval copywriter-agent
/novais-digital:eval designer-agent
```
