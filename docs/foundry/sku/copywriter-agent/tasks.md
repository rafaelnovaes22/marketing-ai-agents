...
## Wave 3 — Test RED (TDD-first, Foundry-10)

> ⚠️ **Foundry-10 obriga:** test_agent em mode=red gera testes ANTES de Wave 4 (build refinado).
> Operador roda `npm test` e CONFIRMA QUE FALHA antes de prosseguir.

- [x] **T3.1** `/novais-digital:aios-run copywriter-agent --step=test --mode=red` (gera tests/copywriter-agent/{unit,integration,e2e}/) — 30min — dev/agent — Wave 2 estruturas prontas — ✅ 2026-05-19 (3 arquivos RED: handoff-contract + re-roll-by-block + voice-drift-cancel)
...
- [ ] **T5.1** `/novais-digital:aios-run copywriter-agent --step=test --mode=verify` (cobertura + gaps) — 30min — agent — Wave 4
...
- [ ] **T5.6** CI: workflow `foundry-test` + `foundry-eval` ativos para este SKU — 1h — dev — T5.4
...
- [ ] **T6.1** `/novais-digital:sla-threshold copywriter-agent` (define SLA contratual ≤900s por tipo) — 30min — dev/@po-guardian
...
- [ ] **T6.7** `/novais-digital:promote copywriter-agent --to=shadow` (promotion-officer assina) — 30min — agent/@promotion-officer — todos anteriores
...
→ Criar testes RED **antes** do build final (Gate G6 Foundry-10)
