...
**5 dias de implementação** seguindo pipeline AIOS TDD-first (Foundry-10):
...
| Item | Stack |
|------|-------|
| `/novais-digital:sla-threshold copywriter-agent` (define SLA contratual ≤900s) | Documento + workflow |
...
| Handoff contract (output JSON) documentado para Designer Agent + Webflow publisher | `docs/foundry/sku/copywriter-agent/handoff-contract.md` |
| Promoção draft → SHADOW (`/novais-digital:promote copywriter-agent --to=shadow`) | promotion-officer assina |
...
- `bash scripts/foundry doctor` no projeto (consumer mode) sem fails não-esperados
- Eval-suite roda em CI (workflow `foundry-eval`)
...
# 2. Schema (Prisma + Zod)
/novais-digital:aios-run copywriter-agent --step=schema

# 3. Test RED (testes ANTES do código)
/novais-digital:aios-run copywriter-agent --step=test --mode=red
# Operador roda `npm test` e CONFIRMA QUE FALHA

# 4. Build (apenas backend — copywriter-agent não tem frontend dedicado em fase 1)
/novais-digital:aios-run copywriter-agent --step=build

# 5. Test VERIFY (após build)
/novais-digital:aios-run copywriter-agent --step=test --mode=verify

# 6. Review final
/novais-digital:aios-run copywriter-agent --step=review
```
...
| Integration tests presentes | Gate G6 (Foundry-10) | ≥ 6 testes (2 por output_type) |
...
| Eval-suite pass rate | `/novais-digital:eval` | ≥ 85% |
| Critical path eval | `/novais-digital:eval` | 100% (5/5) |
...
→ `/novais-digital:tasks copywriter-agent` (decomposição em waves executáveis)
