...
- [ ] **T1.1** Solicitar acesso Meta Business Manager Novais Digital + criar test ad account sandbox — 1h — founder/dev — sem deps
...
> ⚠️ Foundry-10 obriga: test_agent em mode=red gera testes ANTES de Wave 4. Operador roda `npm test` e CONFIRMA QUE FALHA.

- [ ] **T3.1** `/novais-digital:aios-run trafego-agent --step=test --mode=red` (gera tests/trafego-agent/{unit,integration,e2e}/) — 30min — dev/agent
...
- [ ] **T5.1** `/novais-digital:aios-run trafego-agent --step=test --mode=verify` (cobertura + gaps) — 30min — agent
...
- [ ] **T5.6** CI: workflow `foundry-test` ativo para este SKU (excluir sandbox Meta em PR; rodar nightly) — 1h — dev

**Total Wave 5:** ~5h

## Wave 6 — Ship + Promoção SHADOW (1 dia)

- [ ] **T6.1** `/novais-digital:sla-threshold trafego-agent` — definir SLA contratual 300s SETUP (não tempo Meta aprovar) — 30min — dev
...
- [ ] **T6.7** `/novais-digital:promote trafego-agent --to=shadow` (promotion-officer assina) — 30min — agent
