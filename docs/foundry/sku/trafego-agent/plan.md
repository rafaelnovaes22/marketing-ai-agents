...
**~4 dias úteis** seguindo pipeline AIOS TDD-first (Foundry-10):
...
| Item | Stack |
|------|-------|
| SLA threshold definido (`/novais-digital:sla-threshold` — 300s SETUP, não tempo Meta aprovar) | Documento + workflow |
...
- Primeira campanha publicada em SHADOW (conta Novais Digital própria, sem cobrar)
- Langfuse mostra trace completo + custo breakdown
- Bandit cycle 1 dispara automaticamente em t=4h

## 8. Workflow AIOS TDD-first (Foundry-10)

```bash
# 1. Spec + unit-economics aprovados (✅)
# 2. Schema Prisma (extensão)
/novais-digital:aios-run trafego-agent --step=schema

# 3. Test RED (testes antes do código)
/novais-digital:aios-run trafego-agent --step=test --mode=red

# 4. Build
/novais-digital:aios-run trafego-agent --step=build

# 5. Test VERIFY
/novais-digital:aios-run trafego-agent --step=test --mode=verify

# 6. Review final
/novais-digital:aios-run trafego-agent --step=review
```
...
> **NÃO mede** quanto Meta demora para fazer `policy_review` (pode ser 0s a 24h, fora do controle Novais Digital).
...
→ `/novais-digital:tasks trafego-agent` (decomposição em waves)
