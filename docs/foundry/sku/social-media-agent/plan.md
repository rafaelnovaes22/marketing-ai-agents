...
**4 dias de implementação** seguindo pipeline AIOS TDD-first (Foundry-10):
...
| SLA threshold definido (`/novais-digital:sla-threshold`) | Documento + workflow |
...
- ✅ `bash scripts/foundry doctor` no projeto (consumer mode) sem erros críticos
...
# 2. Schema (Prisma)
/novais-digital:aios-run social-media-agent --step=schema
...
/novais-digital:aios-run social-media-agent --step=build

# 5. Test VERIFY (após build)
/novais-digital:aios-run social-media-agent --step=test --mode=verify
# Veredicto: TESTES SUFICIENTES ou ADICIONAR TESTES

# 6. Review final
/novais-digital:aios-run social-media-agent --step=review
...
| E2E Playwright (publicação real) | CI workflow `foundry-test` | 3 happy paths |
...
|-------|---------|-----------|
| TDD red phase atrasa Fase 1 (dev acostumado a code-first) | +0.5 dia | Pair programming primeira sessão; usar `/novais-digital:aios-run --step=test --mode=red` |
...
→ `/novais-digital:tasks social-media-agent` (decomposição em ClickUp)
