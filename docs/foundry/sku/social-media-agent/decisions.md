...
> ADRs específicos deste SKU. Decisões globais do projeto ficam em `docs/foundry/decisions.md` raiz (a criar quando aparecerem). Convenção de IDs: **ADR-NNN-DS** (DS = Novais Digital Social).
...
Stack atual: Claude Sonnet 4.6 + Imagen 4 + Ideogram v2 + Zernio + Langfuse. Foundry C7 exige domain layer sem acoplamento a SDK específico.
