...
> ⚠️ **Foundry-10 obriga:** test_agent em mode=red gera testes ANTES de Wave 4 (build).
> Operador roda `npm test` e CONFIRMA QUE FALHA antes de prosseguir.

- [ ] **T3.1** `/novais-digital:aios-run --step=test --mode=red` (gera tests/social-media-agent/{unit,integration,e2e}/) — 30min — dev/agent
...
- [ ] **T5.1** `/novais-digital:aios-run --step=test --mode=verify` (cobertura + gaps) — 30min — agent
...
- [ ] **T5.5** CI: workflow `foundry-test` ativo para este SKU — 1h — dev
...
- [ ] **T6.3** `/novais-digital:sla-threshold social-media-agent` (define SLA contratual) — 30min — dev
...
- [ ] **T6.7** `/novais-digital:promote social-media-agent --to=shadow` (promotion-officer assina) — 30min — agent
