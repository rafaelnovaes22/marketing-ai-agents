...
> ⚠️ **Foundry-10 obriga:** test_agent em mode=red gera testes ANTES da Wave 4. Operador confirma falha.

- [x] **DES-T3.1** `/novais-digital:aios-run --step=test --mode=red` (gera `tests/designer-agent/{unit,integration,e2e}/`) — 30min — dev/agent — Wave 2 stable — ✅ 2026-05-19 (3 arquivos RED: cost-cap + partial-recovery + handoff-contract)
...
- [ ] **DES-T5.1** `/novais-digital:aios-run --step=test --mode=verify` (cobertura + gaps) — 30min — agent
- [ ] **DES-T5.2** CI workflow `foundry-test` + `foundry-eval` ativo para designer-agent — 1h — dev
- [ ] **DES-T5.3** `/novais-digital:sla-threshold designer-agent` (SLA 20min, brand ≥99 individual, max retries=1, max cost R$3) — 30min — dev
...
- [ ] **DES-T5.8** `/novais-digital:promote designer-agent --to=shadow` (promotion-officer assina) — 30min — agent
