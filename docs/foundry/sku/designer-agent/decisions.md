...
> Decisões globais do projeto vão em `docs/foundry/decisions.md` raiz (quando aparecerem).
...
O `brand/novais-digital-brand-guide.yaml` declara explicitamente:
```yaml
tolerance:
  exact_match_required: 99
```

Foi decisão **direta do founder** (não derivada de benchmark de mercado). A justificativa estratégica é que **brand consistency é o principal moat do Novais Digital Social** vs concorrência genérica (Canva AI, Adobe Firefly), portanto qualquer slide off-brand é defeito.
...
Cenário típico de carrossel Novais Digital (7 slides):
...
3. **Briefing problemático** (ex: requer cor fora da paleta Novais Digital) — não corrige; degrade gracefully
...
- ⚠️ Em SHADOW, calibrar prompt do re-roll (ênfase em "Novais Digital brand strict palette + Inter typography")
...
- **Numeração:** ADR-NNN-DES sequencial (DES = Designer, vs DS = Novais Digital Social geral)
