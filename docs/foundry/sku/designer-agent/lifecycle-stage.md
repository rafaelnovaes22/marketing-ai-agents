...
|-------|------|-----------|-----------------|-------|
| `(new)` → `draft` | 2026-05-13 | @po-guardian (pending review) | Diagnose + spec + unit-economics + plan + tasks + eval-cases criados | 2º SKU do Novais Digital Social (após social-media-agent). Reaproveita Wave 2 do social-media (adapters image-gen + brand validator). |
...
4. [ ] Eval-suite com ≥ 88% pass rate (`/novais-digital:eval designer-agent`)
...
**Comando para transição:** `/novais-digital:promote designer-agent --to=shadow`
...
**Comando:** `/novais-digital:promote designer-agent --to=assisted`

**Pré-requisito adicional:** `/novais-digital:sla-threshold designer-agent` define SLA contratual (20min + brand ≥99 + max cost R$3).
...
- Novais Digital própria (cliente piloto) endossa qualidade
...
- `/novais-digital:promote designer-agent --to=shadow` (de ASSISTED ou AUTONOMOUS)
- `/novais-digital:promote designer-agent --to=draft` (de SHADOW)
