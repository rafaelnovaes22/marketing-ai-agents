...
diagnose_command: "/novais-digital:diagnose designer-agent"
...
> "Entregar um carrossel de design completo (7 slides JPG 1080×1080) seguindo o brand Novais Digital com score ≥99% no validator multimodal, em ≤20 minutos, pronto para o social-media-agent (ou cliente direto) acoplar caption e publicar."
...
**Cabe no ICP?** ✅ Sim. Novais Digital Social posicionado como SaaS² para founders e times que precisam de **alto volume de design on-brand sem time interno**.
...
- Multi-tenant em fase 1 (brand_guide hard-coded para Novais Digital; multi-tenant na wave 2).
...
- **Single-tenant fase 1 (C8):** brand_guide.yaml hard-coded para Novais Digital. Multi-tenant via `brand_guides/{tenant_id}.yaml` entra na wave 2.
...
- **Brand guide já existe** em `brand/novais-digital-brand-guide.yaml` (extração validada do site oficial Novais Digital, confidence: high). O agente CONSUMEmais não cria/edita.
- **Threshold de brand é POLÍTICA EXPLÍCITA** em `brand/novais-digital-brand-guide.yaml` (`tolerance.exact_match_required: 99`). Foi decisão do founder. Mudança exige ADR-001-DES.
...
→ `/novais-digital:spec --type=platform-sku designer-agent`
