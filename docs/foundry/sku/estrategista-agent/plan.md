...
| Mixpanel SANDBOX setup (tenant fake Novais Digital) | Mixpanel project token dev | B |
...
| Test RED via `/novais-digital:aios-run --step=test --mode=red` | Foundry-10 mandatory |
...
| SLA threshold (`/novais-digital:sla-threshold estrategista-agent`) | 120s P95 |
...
| Promoção draft → SHADOW (`/novais-digital:promote`) | promotion-officer assina |

### Output esperado fim Fase 4

- ✅ Primeiro diagnóstico real publicado em SHADOW (Novais Digital própria como tenant piloto)
...
## 9. Workflow AIOS TDD-first (Foundry-10)
...
# 2. Schema
/novais-digital:aios-run estrategista-agent --step=schema

# 3. Test RED
/novais-digital:aios-run estrategista-agent --step=test --mode=red
# Operador roda `npm test` → CONFIRMA QUE FALHA

# 4. Build
/novais-digital:aios-run estrategista-agent --step=build

# 5. Test VERIFY
/novais-digital:aios-run estrategista-agent --step=test --mode=verify

# 6. Review final
/novais-digital:aios-run estrategista-agent --step=review
```
...
|------|-------------|:------:|
| Coverage line (unit) | CI `foundry-test` | ≥ 85% |
| Coverage branch (unit) | CI `foundry-test` | ≥ 80% |
| Integration tests presentes | Gate G6 (Foundry-10) | ≥ 5 testes (incluindo Mixpanel sandbox real) |
...
→ `/novais-digital:tasks estrategista-agent` (decomposição em 5 Waves)
