...
- [ ] **T1.3** Seed: 1 tenant fake Novais Digital com event mapping YAML — 30min — dev — T1.2
...
- [ ] **T1.17** Smoke: `MixpanelAdapter.countActiveUsers('novais-digital-fake', 30)` retorna número real — 30min — dev — T1.5, T1.16
...
## Wave 3 — Test RED (TDD-first, Foundry-10) — INSERIR ANTES DO BUILD COMPLETO

> ⚠️ **Foundry-10 obriga:** test_agent em mode=red gera testes ANTES de Wave 4 finalização.

- [ ] **T3.1** `/novais-digital:aios-run --step=test --mode=red` (gera tests/estrategista-agent/{unit,integration,e2e}/) — 30min — dev/agent
...
- [ ] **T4.3** Integration test: Mixpanel SANDBOX real (tenant fake Novais Digital, 1 cenário end-to-end) — 1.5h — dev — T1.16
...
- [ ] **T5.5** `/novais-digital:aios-run --step=test --mode=verify` (gaps + coverage) — 30min — agent — T4.4
- [ ] **T5.6** Eval CLI `npm run eval estrategista-agent` (roda 22 cases) — 30min — dev — T5.4
- [ ] **T5.7** CI workflow `foundry-test` ativo para este SKU — 30min — dev — T5.6
- [ ] **T5.8** `/novais-digital:sla-threshold estrategista-agent --p95=120s` — 30min — dev — sem deps
...
- [ ] **T5.13** `/novais-digital:promote estrategista-agent --to=shadow` (promotion-officer assina) — 30min — agent — Wave 4 + 5
